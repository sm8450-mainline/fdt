...
class FdtPropertyWords(FdtProperty):
    ...
    def dts_represent(self, depth=0):
        """Get dts string representation"""
        # Guess if this is a phandle reference
        if (
            (len(self.words) == 1 or (len(self.words) == 3 and "gpio" in self.name))
            and self.words[0] > 128  # TODO: Surely we can do this better?
            and (node_name := PHANDLES.get(self.words[0]))
        ):
            for node_path, real_node_name in SYMBOLS:
                if node_path.endswith(node_name):
                    return INDENT * depth + self.name + " = <&" + real_node_name + ">;"
        return (
            INDENT * depth
            + self.name
            + " = <"
            + " ".join(
                [
                    (
                        "0x%x" % word
                        if (word > 1024 and "volt" not in self.name)
                        or "clocks" in self.name
                        or "interrupts" in self.name
                        or "interconnects" in self.name
                        or "phandle" in self.name
                        else str(word)
                    )
...
class FdtPropertyBytes(FdtProperty):
    ...
    def dts_represent(self, depth=0):
        ...
        return (
            ...
            + self.name
            + " = ["
            + " ".join(["%02x" % (byte & int("ff", 16)) for byte in self.bytes])
            + "];"
        )
...
class FdtNode(object):
    ...
    def add_raw_attribute(self, name, raw_value):
        ...
        property = FdtProperty.new_raw_property(name, raw_value)
        if name == "phandle":
            PHANDLES[property.words[0]] = self.name
        else:
    ...
    def dts_represent(self, depth=0):
        ...
        result = ("\n").join(
            [
                sub.dts_represent(depth + 1)
                for sub in self.subdata
                if type(sub) != FdtNop and getattr(sub, "name") != "phandle"
            ]
        )
        if len(result) > 0:
            result += "\n"
        ...
        name = self.name
        for path, real_name in SYMBOLS:
            if path.split("/")[-1] == self.name:
                name = f"{real_name}: {self.name}"
...
class Fdt(object):
    ...
    def populate_symbols(self):
        if self.rootnode is not None:
            for subnode in self.rootnode.subdata:
                if subnode.name == "__symbols__":
                    for node in subnode.subdata:
                        SYMBOLS.append((node.strings[0], node.name))
